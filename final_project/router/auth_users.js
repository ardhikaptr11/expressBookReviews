...
regd_users.put("/auth/review/:isbn", (req, res) => {
	...
	if (books[ISBN]) {
		...
		return res.status(200).send(`Review successfully added/updated to book with ISBN ${ISBN}`);
	} else {
...
regd_users.delete("/auth/review/:isbn", (req, res) => {
	...
	if (books[ISBN]) {
		...
		return res
			.status(200)
			.send(`Review posted for book with ISBN ${ISBN} has been deleted by the user`);
	} else {
		return res.status(403).json({ message: "Book not found" });
	}
});
